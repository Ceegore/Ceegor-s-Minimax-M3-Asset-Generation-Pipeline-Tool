...
rem (<base>.sha256.minisig), the pinned public key (minisign.pub) and the
rem minisign verifier are ALL mandatory - a release without a verifiable
rem signature is refused outright. An attacker who can replace the archive
rem can also replace the plaintext .sha256 file, so the hash alone proves
rem nothing. Verifier resolution: %MINIMAX_MINISIGN_TOOL% env override,
rem then minisign.exe beside this installer (shipped with the release),
rem then PATH (https://jedisct1.github.io/minisign/).
