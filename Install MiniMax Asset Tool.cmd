...
powershell.exe -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop'; $root=$env:MINIMAX_INSTALL_DIR_FOR_HASH; $minEntries=if($env:MINIMAX_MANIFEST_MIN_ENTRIES){[int]$env:MINIMAX_MANIFEST_MIN_ENTRIES}else{50}; $manifest=Join-Path $root 'FILES.sha256'; $bad=0; $entries=@{}; foreach($line in [IO.File]::ReadAllLines($manifest)){ if([string]::IsNullOrWhiteSpace($line)){continue}; if($line -match '^([0-9a-fA-F]{64})\s+(.+)$'){ $rel=$matches[2].Trim(); if($entries.ContainsKey($rel.ToLowerInvariant())){Write-Host ('  DUPLICATE: '+$rel); $bad++; continue}; $entries[$rel.ToLowerInvariant()]=$rel; $fp=Join-Path $root $rel; if(-not [IO.File]::Exists($fp)){Write-Host ('  MISSING: '+$rel); $bad++; continue}; $hs=[System.Security.Cryptography.SHA256]::Create(); $fs=[IO.File]::OpenRead($fp); try{$hb=$hs.ComputeHash($fs)}finally{$fs.Dispose()}; $actual=[BitConverter]::ToString($hb).Replace('-','').Trim().ToLowerInvariant(); if($actual -ne $matches[1].ToLowerInvariant()){Write-Host ('  TAMPERED: '+$rel); $bad++} } else {Write-Host ('  MALFORMED manifest line: '+$line); $bad++} }; foreach($fp in [IO.Directory]::EnumerateFiles($root,'*',[IO.SearchOption]::AllDirectories)){ $rel=$fp.Substring($root.Length+1).Replace('\','/'); if($rel -eq 'FILES.sha256'){continue}; if(-not $entries.ContainsKey($rel.ToLowerInvariant())){Write-Host ('  UNLISTED: '+$rel); $bad++} }; if($entries.Count -lt $minEntries){Write-Host ('  MANIFEST TOO SMALL: only '+$entries.Count+' entries (a real release has hundreds).'); $bad++}; if($bad -gt 0){throw ($bad.ToString()+' file(s) failed the completeness/integrity check.')}"
if errorlevel 1 (
  echo.
