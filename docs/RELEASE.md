...
After the build, the pipeline finalizes the release inventory (`npm run finalize:release`, V104-C002) so the manifest covers EVERY shipped file — archives, installer, provenance, SBOM, `minisign.pub`, and the pinned Minisign verifier — and then signs it. Verification is strict and fail-closed, in three explicit modes: `npm run verify:release:legacy` (archive hashes, Minisign manifest signature, provenance, SBOM — no Authenticode claim) for the final legacy-compatible 1.0.x transition release, `npm run verify:release:signpath` (adds `--require-authenticode`) for SignPath-signed 1.1.x releases, and the legacy strict default `npm run verify:release`. The provenance record must show the expected commit and `commitDirty: false`. Test the extracted ZIP on a clean Windows account before publishing it.
...
The current public release does not yet carry a new project Authenticode signature. The release integrity chain is fail-closed (V104-C001/V104-C002) and documented honestly:
...
- **Legacy-compatible transition (final 1.0.x)**: keeps the previously validated Windows runtime binaries byte-identical (hash-locked) and updates the application content through Electron's `resources/app.asar`. It is verified with `verify:release:legacy` and makes no Authenticode claim. This mechanism is used for exactly one release.
- **SignPath (1.1.x)**: future releases are built fresh from source in GitHub Actions and submit only the project-owned `MiniMaxAssetTool.exe` for managed Authenticode signing; `verify:release:signpath` enforces the signature. Upstream binaries (Electron DLLs, FFmpeg, Real-ESRGAN, ONNX, Minisign) are never signed with the project identity.

Do not promise that SmartScreen, Defender, AppLocker, or company security policy will allow the app without a warning. Publish the SHA-256 inventory manifest so users can verify that their download matches the release artifact. Never tell users to disable Defender, add an exclusion, change execution policy, or bypass an organization policy.

The included CMD installer deliberately uses a per-user directory under `%LOCALAPPDATA%`, needs no elevation, makes no network requests, and creates shortcuts with Windows' built-in tools. It is an install helper, not a security-warning bypass.
