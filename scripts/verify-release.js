...
function verifySbom(root, paths) {
  ...
  if (isCycloneDX) {
    ...
    if (!Array.isArray(sbom.components) || sbom.components.length === 0) {
      ...
      // V104-H003: a structurally valid but INCOMPLETE SBOM must fail.
      errors.push(...verifySbomCompleteness(root, sbom));
    }
  } else if (isSPDX) {
...
/**
 * V104-H003: fail-closed SBOM COMPLETENESS, not just schema/name/version.
 * The v1.0.4 requalification rejected a verifier that accepted a SBOM built
 * from `npm ls --depth=0` (direct deps only, fail-open generator). A
 * release SBOM must:
 *   1. declare the full-transitive marker emitted by the generator,
 *   2. list EVERY production package resolved by package-lock.json with its
 *      exact locked version,
 *   3. list EVERY offline runtime asset from scripts/runtime-assets.json
 *      with the manifest's pinned SHA-256.
 * Any unreadable input (lockfile, asset manifest) is itself a failure.
 */
function verifySbomCompleteness(root, sbom) {
  const errors = [];
  const props = (sbom.metadata && sbom.metadata.properties) || [];
  const propVal = (name) => {
    const p = props.find((x) => x && x.name === name);
    return p ? p.value : null;
  };
  if (propVal('sbom:dependencyTree') !== 'full-transitive') {
    errors.push('SBOM does not declare the "full-transitive" dependency-tree marker (sbom:dependencyTree) — it was generated by an incomplete enumerator.');
  }

  // 2. Every production package in the lockfile must appear with its exact
  //    locked version. The lockfile is the ground truth of what the build
  //    installed; a SBOM missing any of those entries is incomplete.
  const lockPath = path.join(root, 'package-lock.json');
  let lock;
  try {
    lock = JSON.parse(fs.readFileSync(lockPath, 'utf8'));
  } catch (e) {
    return errors.concat(`cannot read package-lock.json for the SBOM completeness check: ${e.message}`);
  }
  ...
  if (!lockPackages) {
    ...
    for (const [key, entry] of Object.entries(lockPackages)) {
      ...
      if (entry && entry.dev === true) continue;      // dev-only packages are not shipped
      // Optional packages pinned to OTHER platforms are not installed by
      // `npm ci` on the win32/x64 release runner and are not shipped.
      if (entry && entry.optional === true) {
        const osOk = !Array.isArray(entry.os) || entry.os.includes('win32');
        const cpuOk = !Array.isArray(entry.cpu) || entry.cpu.includes('x64');
        if (!osOk || !cpuOk) continue;
      }
    ...
    if (missing.length > 0) {
      ...
      errors.push(`SBOM omits ${missing.length} production package(s) resolved by package-lock.json: ${sample}${missing.length > 10 ? `, … and ${missing.length - 10} more` : ''}.`);
    }
  }

  // 3. Every offline runtime asset (executables, native DLLs, ML models)
  //    must be inventoried with the manifest's pinned SHA-256.
  const manifestPath = path.join(root, 'scripts', 'runtime-assets.json');
  let manifest;
  try {
    manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  } catch (e) {
    return errors.concat(`cannot read scripts/runtime-assets.json for the SBOM completeness check: ${e.message}`);
  }
  const assetComponents = (sbom.components || []).filter((c) => c && Array.isArray(c.properties)
    && c.properties.some((p) => p && p.name === 'asset:path'));
  for (const asset of manifest.assets || []) {
    const comp = assetComponents.find((c) => c.properties.some((p) => p.name === 'asset:path' && p.value === asset.path));
    if (!comp) {
      errors.push(`SBOM omits runtime asset ${asset.path}.`);
      continue;
    }
    const sha = (comp.hashes || []).find((h) => h && h.alg === 'SHA-256');
    if (!sha || String(sha.content).toLowerCase() !== String(asset.sha256).toLowerCase()) {
      errors.push(`SBOM runtime asset ${asset.path} carries a SHA-256 that does not match the runtime manifest.`);
    }
  }
  return errors;
}
...
module.exports = { evaluate, parseArgs, signatureFor, writeManifest, verifyArchiveIntegrity, verifyManifest, verifyProvenance, verifyArchiveFreshness, verifySbom, verifySbomCompleteness, validateArchiveSequence, validatePEHeader, findBinariesRecursive };
