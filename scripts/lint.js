...
const SIZE_BUDGETS = new Map([
  ['main/ipc/registerConfigIpc.js', 463], // _5 audit: H-052 externalToolsEnabled flag in _publicConfig (+4), B-007 apiKeyAction keep|replace|clear typed secret command + H-046 batch_max_units in _publicConfig (+65)
  ['main/ipc/registerFileBrowserIpc.js', 346], // 360-audit: HIGH-015 grantId enforcement on reveal/openInExplorer (+9)
  ['main/ipc/registerMmxIpc.js', 400], // _5 audit: B-002 readGrantIds plural + H-046 pre-spawn batch_max_units cost gate + H-040 authTestDraft registration (+5)
  ['renderer/app.js', 2159], // _5 audit: H-051 typed state-save promise + cancel resolves waiters (+14), H-050 fail-closed existence checks + all-errors throw (+8), H-048 single-file audio-cutter gate replacing the misleading bulk-trim (+2), H-044 saveAllStates jobsArchived splice + warnings surfacing, H-046 batch_max_units boot reconstruction (+48), H-029 fbReveal grant (+1). DECOMPOSITION TARGET — do NOT bump further. Planned split: shortcut/keydown handling → renderer/services/shortcutRegistry.js, gen-poller + batch wiring → renderer/jobs/, style/import helpers → renderer/sections/. History: PE-018 shortcut-scope guard. BGR-002 CapabilityGuard boot order (+13). BUG #3 gen-poller wiring (+18). BUG #5 Ctrl+Enter scoping (+3). GEW-002 fbMove/fbCopy dual-grant (+5). kkkook1 M3 chat + reset/relaunch + import (+80). h7-fixes cancelPendingStateSave (+9). Premade styles import (+35).
  ...
  ['renderer/jobs/JobRunner.js', 505], // _5 audit: H-044 archive-starvation fix — cap pre-trim removed, HARD_BOUND runaway bound + explanatory comment (+2)
  ...
  ['renderer/pipeline/pipelineCard.js', 485], // P3.5 (DA-H-008/012): Back/Finalize + operationId lock moved to pipelineCardMutations.js (ratchet-down from 531).
  ['renderer/pipeline/pipelineOps.js', 429], // P3.5 (DA-H-010): file-placement helpers moved to pipelineFileOps.js (ratchet-down from 485).
  ['renderer/sections/section03_Settings_tab_panes.js', 982], // _5 audit: H-052 external-tools disabled notice gate (+12), H-040 draft-key test (+11), H-041 readonly path inputs (+3), H-038 commitState transaction (+2), B-007 apiKeyAction (+22). R1.2a: pickFolderFull (+10). kkkook1: M3 chat (+33). h7-fixes (+9). KGO-022 (+3). Premade styles (+35).
  ...
  ['renderer/services/fileBrowser1.js', 1005], // _5 audit: H-047 shared matchesFileBrowserFilters predicate + currentFbFilterState so fbSelectAll honours the live text search + type filter (+24), H-029 fbReveal grant (+1). KGO2-012/018: container contextmenu + image viewer double-click (+15). // KGO2-012/018: container contextmenu + image viewer double-click (+15).
  ...
  ['renderer/specs/modelSpecs.js', 606], // _5 audit: H-057 import-time tool-flag validation (crop/resize WxH, optimize/upscale/removebg enums, trim pairing) in validateValues (+36), H-002 duration enumCheck (+2), H-003 FL2V mode resolution filtering (+19), H-003 validateValues FL2V check (+5)
  ['renderer/tabs/batchImportHelper.js', 1295], // _5 audit: H-055 throwing validator → _defective rows (+5). KGO2-004: fenced batch-json import count calculation (+14). T3/T4/T6 (batchgen-ux): combined all-types confirmation modal (per-type paid-call counts, output-folder picker, subfolder opt-out checkbox) replaces the plain asyncConfirm in startAllBatchGen (+64).
  ['renderer/tabs/batchManager.js', 866], // _5 audit: H-056 partial-status counter + genLastPostprocessErrors channel (+28). H9-004/005/013/016/017/018 + H11-1C/3 + X3-01 + B.1: batch-owned grant minting for pure-batch flows. BGR-011/017/023: pipeline spellings + concurrent globals + stale grant (+7). BUG #5: `.actions` selector scoping (+5). gewv2 GEW-009/011: upscaleModel + removeBackgroundUseGpu rowPostprocess fields (+7). T3-T7 (batchgen-ux): startBatchGen opts + computeExpectedCalls extraction + per-type subfolder routing (+59).
  ['renderer/tabs/imageTab.js', 1126], // _5 audit: B-002/B-003 read-grant minting + https-vs-local input typing for image refs (+15). 360-audit: C-007 magic-byte admission + grant flow hardening (+14)
  ['renderer/tabs/musicTab.js', 687], // _5 audit: B-002 read-grant minting for local input refs (+10). H9-018 + R7.4: lyrics/instrumental/lyrics-optimizer controls + model-change disable + preflight fix. H3-B7: image viewer overlay extracted to overlays/imageViewerOverlay.js (-285 lines). R7.2b.J: isSubcommandAvailable('music') subcommand guard in Generate handler (+9). kkkook1: minor UI tweaks (+7).
  ['renderer/tabs/speechTab.js', 584], // _5 audit: B-002 read-grant minting for local input refs (+7). GEN-001: capability guard disables unsupported controls. R7.5: mmx grant threading + language-help reflow. R7.2b.J: isSubcommandAvailable('speech') subcommand guard in Generate handler (+7). kkkook1: minor UI tweaks (+7).
  ['renderer/tabs/videoTab.js', 532], // _5 audit: B-002 read-grant minting for the first-frame ref (+5). GEN-001/002: capability guard + model auto-resolution. BGR-001/003/004: flag ordering + fbExists grant (+6). R7.2b.J: isSubcommandAvailable('video') guard (+2). kkkook1: footer layout change (+4). H-003: mode-aware resolution dropdown (+15). H-004: effectiveModel before preflight (+11).
  ...
  ['src/mmx.js', 492], // H9-003: probeMmxVersion + compareSemver + SUPPORTED_MMX. R2.4: proc-tracking extracted to mmxProcTracker.js (−50).
  ['src/state.js', 465], // _5 audit: H-045 archive-first jobs transaction (session dedupe Set + append-before-trim + non-persisted _jobsArchived/_archiveWarnings) (+34). PE-014: resolveAutoBestModel require for auto-best fallback. PE-031: imageEditorPrefs persistence whitelist with sanitization.
