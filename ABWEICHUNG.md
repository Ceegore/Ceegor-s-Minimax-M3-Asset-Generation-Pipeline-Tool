# ABWEICHUNG — dokumentierte Abweichungen vom Umsetzungsplan

Gemäß `_UMSETZUNGSPLAN_SIGNING_MIGRATION.md` §0.1.10 wird hier jede notwendige
Abweichung von der Vorgabe festgehalten.

## A-001 — Installer verzichtet auf die nachgelagerte Authenticode-Erzwingung

- **Ursprüngliche Vorgabe:** Der Root-Installer (`Install MiniMax Asset Tool.cmd`)
  verwarf nach der Extraktion jede `MiniMaxAssetTool.exe`, deren
  `Get-AuthenticodeSignature`-Status nicht `Valid` war („A genuine release is
  code-signed“).
- **Technische Ursache:** Es existiert aktuell kein einziges Authenticode-signiertes
  Release; die Behauptung war unwahr (Plan §2.5, §9). Der finale
  Legacy-Übergangsrelease 1.0.7 ist ausdrücklich nicht neu Authenticode-signiert
  (Plan §11, §16.7), der Check hätte jede legitime Installation abgebrochen.
- **Risiko:** Ein signierter 1.1.x-Kandidat wird vom Installer nicht zusätzlich
  auf Authenticode geprüft. Restrisiko wird durch die weiterhin erzwungene
  Minisign-Manifestsignatur + SHA-256-Inventar und die Acceptance-Prüfung nach
  Testextraktion (Plan §32.1) abgedeckt.
- **Gewählte Alternative:** Der Installer prüft nach der Extraktion nur noch die
  Existenz der Haupt-EXE; Vertrauensanker bleibt die gepinnte Minisign-Signatur
  des Inventar-Manifests (RR2-C001).
- **Zusätzliche Tests:** Bestehende Installer-Trust-Anchor-Tests
  (`tests/unit/scripts/installerTrustAnchor.rr2.test.js`,
  `tests/unit/scripts/verifyRelease.test.js`) laufen weiter grün; die
  Legacy-Acceptance installiert den 1.0.7-Kandidaten real.
- **Abnahmeentscheidung:** Vom Projektauftraggeber genehmigt (Planfreigabe
  „Signing Migration: v1.0.7 Legacy Final + v1.1.0 SignPath Readiness“).
  Mit dem ersten SignPath-Release (1.1.0) erhält der Installer eine
  modusabhängige Authenticode-Prüfung zurück.

## A-002 — Seed-Erfassung über bereinigte Staging-Kopie statt Originalverzeichnis

- **Ursprüngliche Vorgabe:** `capture-legacy-shell-lock.js` wird direkt gegen
  `C:\Tools\MinimaxAssetTool1.0.0` ausgeführt (Plan Phase 0, Schritt 7).
- **Technische Ursache:** Das installierte Arbeitsverzeichnis enthält lokale
  Nutzerzustände: `config.txt` (inkl. eines echten API-Schlüssels),
  `state.json` und `batches.json`. Der Guard `FORBIDDEN_USER_PATHS` verweigert
  `config.txt` zu Recht; zusätzlich dürfen Nutzerzustände weder eingefroren
  noch exportiert werden (Plan: Seed-Export ohne Secrets/Nutzerdaten).
- **Gewählte Alternative:** Eine Staging-Kopie
  (`C:\Projects\MiniMaxAssetTool-seed-staging-clean`) ohne die drei
  Nutzerzustandsdateien diente als Seed-Quelle. Alle übrigen Dateien sind
  byteidentisch zum Original; der Lock (76 frozen files, 22 PE-Dateien,
  EXE-SHA-256 `f812621e…baba8c`) wurde am 2026-08-04 daraus erfasst. Der
  Export (`C:\Projects\MiniMaxAssetTool-legacy-seed-export-2026-08-04`,
  87 Dateien) wurde auf Secrets/Nutzerdaten geprüft: keine Treffer
  (Einzeltreffer in `MiniMaxAssetTool.exe` ist eine Byte-Koinzidenz in der
  stock Electron-43.1.0-Launcher-PE, die laut früherer Verifikation
  hashgleich zu `node_modules/electron/dist` ist).
- **Risiko:** Keines gegenüber der Vorgabe — die Bereinigung entfernt nur
  Dateien, die ohnehin nie Teil des Locks oder Exports sein durften.
- **Konsequenz für compose:** `state.json`/`batches.json` sind weder im Lock
  noch mutable Präfixe; der Compose-Schritt kopiert sie nicht, der
  Erststart legt sie neu an (entspricht dem Verhalten einer Neuinstallation).

## A-003 — Vorqualifikation: Contract-Skip und Flaky-Acceptance-Opt-out

- **Ursprüngliche Vorgabe:** §16.1 verlangt `npm run test:contract` als
  Pflichtgate; Contract-Skips sind nur „bei dokumentierter externer Quota und
  mit manuellem Ersatztest“ zulässig. Der Flaky-Läufer führt zudem eine
  Installer-Acceptance-Phase gegen ein fertiges Release in `dist-out` aus.
- **Technische Ursache:** Die Contract-Tests rufen die echte MiniMax-API ab
  und verbrauchen bezahlt Credits; das Repository sieht dafür ausdrücklich
  den sanktionierten Skip `MMX_CONTRACT_OPTIONAL=1` vor. Zum Zeitpunkt der
  Vorqualifikation existiert der 1.0.7-Kandidat noch nicht — `dist-out`
  enthält nur Alt-Releases (1.0.1/1.0.2/1.0.4), gegen die die
  Acceptance-Phase keine gültige 1.0.7-Evidenz liefern kann.
- **Gewählte Alternative:** Die Vorqualifikation läuft mit
  `MMX_CONTRACT_OPTIONAL=1` (sanktionierter Skip, Exit 0) und
  `FLAKY_SKIP_ACCEPTANCE=1` (eingebauter Opt-out des Flaky-Läufers).
  Der manuelle Ersatztest ist die Funktionsmatrix §17 (BatchGen je
  Medientyp §17.4) auf dem paketierten 1.0.7-Kandidaten sowie die
  Legacy-Acceptance §16.7 (`test:packaged:release`, `test:installer`,
  `test:acceptance` mit `MINIMAX_RELEASE_MODE=legacy`) nach der Komposition.
- **Risiko:** Kein zusätzlicher Flakiness-Nachweis gegen ein reales Release
  vor der Komposition — dieser wird durch §16.7 auf dem echten Kandidaten
  nachgeholt.
- **Abnahmeentscheidung:** Entspricht den im Skript selbst dokumentierten
  Opt-outs; endgültige Abnahme erfolgt mit den §16.7- und §17-Gates.

## A-004 — Legacy-Kandidat ohne `ffprobe.exe` (Seed-Gleichheit vor Manifest)

- **Ursprüngliche Vorgabe:** `scripts/runtime-assets.json` verlangt
  `ffprobe.exe` in jedem vollständigen Offline-Release (`npm run check`
  prüft die Quelle dagegen grün).
- **Technische Ursache:** Der Legacy-Seed 1.0.0
  (`C:\Tools\MinimaxAssetTool1.0.0\resources\bin`) enthält kein
  `ffprobe.exe` — es wurde erst nach 1.0.0 eingeführt. §14 verlangt exakte
  PE-Gleichheit des komponierten Kandidaten mit dem Lock; eine zusätzliche
  PE würde die Komposition hart abbrechen.
- **Gewählte Alternative:** Der Release-Donor (`build-unpacked.js`)
  verschifft `ffprobe.exe` bewusst nicht; seine Verpackt-Prüfung nutzt eine
  dokumentierte, explizite `skipPaths`-Ausnahme in `verifyRuntimeAssets`
  (nur exakte Manifestpfade, auditierbar im Aufruf). Dev-/QA-Builds nutzen
  weiter den `@ffprobe-installer/ffprobe`-Wrapper aus den Dependencies.
- **Risiko:** Der paketierten Legacy-Kandidat hat kein `resources/bin/ffprobe.exe`;
  Media-Probing läuft über die gebündelten ffmpeg/Wrapper-Pfade der App.
  Die Clean-VM-Acceptance (§18) und die Funktionsmatrix (§17.5/§17.9) auf
  dem echten Kandidaten bestätigen das Verhalten.
- **Konsequenz für Phase 2:** Mit dem SignPath-Release 1.1.0 (neue
  signierte Laufzeit) kehrt `ffprobe.exe` gemäß Manifest in den Releasebaum
  zurück.

## A-005 - Media-Probing-Fallback ueber gesperrtes FFmpeg (Spezifikation 15.1 Prioritaet 1)

- **Urspruengliche Vorgabe:** `main/services/mediaProbe.js` validiert Audio/Video ausschliesslich ueber ffprobe; ohne ffprobe scheitert `probeMedia` hart (und damit jede Audio-/Videogenerierung ueber `ArtifactFinalizer` Schritt 5).
- **Technische Ursache:** Der Legacy-Seed/das Lock 1.0.0 enthaelt kein `ffprobe.exe` (siehe A-004); Abschnitt 14 verbietet jede zusaetzliche PE im komponierten Kandidaten.
- **Gewahlte Alternative:** Abschnitt 15.1 Prioritaet 1 - den vorhandenen kompatiblen FFmpeg-Flow verwenden: `probeMedia` nutzt bei abwesendem ffprobe das bereits im Lock verankerte, hash-gepinnte `ffmpeg-static` ffmpeg.exe (Lock-PE `04e13079`, bereits Bestandteil jeder Laufzeit) mit fester Argumentfolge `-hide_banner -nostdin -analyzeduration 5000000 -probesize 5000000 -i <datei> -t 0 -f null -` und wendet exakt dieselben `validateProbeResult`-Schranken an. Gleiche Sicherheitshuelle: shell:false, fixes argv, 15 s Timeout, 1 MiB Ausgabe-Cap, kein PATH, keine Netzwerkprotokolle. Zusaetzlich verschifft der Donor `@ffprobe-installer` nicht mehr (`package.json` build.files/asarUnpack), damit die Donor-PE-Menge exakt dem Lock entspricht; Dev/QA (`npm start`, Unit-Tests) nutzt die npm-Abhaengigkeit weiter.
- **Risiko:** Das Banner-Parsing erfasst weniger Metadaten als ffprobe-JSON - aber genau die Felder, die die Validierung verwendet (codec_type, Abmessungen, Dauer, Format). Abgedeckt durch drei neue Abschnitt-15.1-Unit-Tests sowie die Abschnitt-17.4/16.7-Matrizen auf dem realen Kandidaten.
- **Konsequenz fuer Phase 2:** Mit 1.1.0 kehrt `ffprobe.exe` gemaess Manifest zurueck; der ffprobe-Primaerpfad von `probeMedia` bleibt unveraendert.
- **Ergaenzung:** Dieselbe dokumentierte, legacy-gated skipPaths-Ausnahme gilt fuer die --package-existing-Pruefung in scripts/zip-portable.js (nur bei MINIMAX_RELEASE_MODE=legacy; normale Releases bleiben fail-closed).


## A-006 - RR2-C001-Stempelmechanik: PowerShell-Einzeiler ohne #-Kommentare (Bugfix Installer-Gate)

- **Befund:** Der Bootstrap-Signaturgate in `Install MiniMax Asset Tool.cmd` ist EIN physischer PowerShell-Einzeiler. Die Stempel-Marker standen als PowerShell-Kommentare (`# RR2-C001-BEGIN/END-EMBEDDED-MINISIGN-PUBKEY`) mitten in dieser Zeile. Ein `#` kommentiert in PowerShell den kompletten Rest der Zeile - dadurch wurde der gesamte Gate-Code nach `$embeddedKeyLines=@()` deaktiviert: Der Installer verifizierte/extrahierte nichts, beendete den PowerShell-Teil mit Exit 0 und scheiterte danach still am fehlenden APP_DIR (`multipart easy install failed` sowie `unsigned-rejection output did not mention the missing signature` in scripts/test-release-installer.js). Der Fehler betraf sowohl das Repo-Template als auch die gestempelte veroeffentlichte Variante (finalize-release-inventory.js liess die Marker in der Zeile stehen).
- **Fix:** Die Marker liegen jetzt in rem-Zeilen ueber dem PowerShell-Aufruf. In der PowerShell-Zeile steht stattdessen der inerte String-Literal-Platzhalter `'RR2-C001-EMBEDDED-MINISIGN-PUBKEY-STAMP-POINT'`, den finalize-release-inventory.js durch die Key-Appends ersetzt. Idempotenz- und Fail-closed-Pruefungen wurden entsprechend umgestellt (Stempelpunkt fehlt + Appends vorhanden = bereits gestempelt; Marker oder Stempelpunkt fehlen = Abbruch).
- **Absicherung:** Zwei neue Regressionstests in tests/unit/scripts/installerTrustAnchor.rr2.test.js verbieten jedes `#` in der Gate-Zeile (Template und gestempelte Fassung). scripts/test-release-installer.js ist wieder vollstaendig gruen (7/7), inklusive Multipart-Installation und Fail-closed-Signaturgate.
- **Sicherheit:** Keine Schwaechung - Trust Anchor, Verifier-Hash-Pin und Fail-closed-Verhalten bleiben identisch; nur die Stempelmechanik ist jetzt syntaktisch korrekt.

## A-007 - CI-Reihenfolge: Minisign-Toolchain vor finalize:release (Workflow-Korrektur)

- **Befund:** In `.github/workflows/release-legacy-final.yml` (Candidate-Job) lief der Schritt `Finalize release inventory` VOR `Prepare pinned Minisign toolchain`. `finalize-release-inventory.js` bricht aber fail-closed ab, wenn `MINISIGN_PUB_PATH` fehlt, und bettet den RR2-C001-Verifier-SHA256-Pin nur ein, wenn `MINISIGN_TOOL_PATH` auf das gepinnte `minisign.exe` zeigt. Beides exportiert erst `scripts/prepare-minisign.ps1` via GITHUB_ENV. In der alten Reihenfolge haette der Candidate-Job am Finalize-Schritt hart abgebrochen bzw. einen Installer ohne aktiven Verifier-Pin veroeffentlicht.
- **Fix:** Die Schritte wurden getauscht: `Prepare pinned Minisign toolchain` laeuft jetzt unmittelbar nach der SBOM-Erstellung, danach `Finalize release inventory` (mit erklaerendem Kommentar im Workflow). `release-gate.yml` ist nicht betroffen - dort werden die Minisign-Umgebungsvariablen ohnehin inline vor finalize vorbereitet.
- **Lokale Verifikation:** finalize wurde lokal mit gesetztem `MINISIGN_PUB_PATH` und `MINISIGN_TOOL_PATH` (gepinntes minisign.exe, SHA256 `6537b1da726d593877dc21720d8f8c44e6c7485da3dfddddee73e8b457e49b1a`) gegen den bei 8c89d58 komponierten Kandidaten ausgefuehrt: Trust Anchor (2 Key-Zeilen) UND Verifier-Pin sind im veroeffentlichten Installer eingebettet, Inventar umfasst 7 Eintraege inklusive minisign.exe und minisign.pub.
- **Sicherheit:** Keine Schwaechung - reine Reihenfolgekorrektur, damit die bereits spezifizierten Fail-closed-Checks ueberhaupt greifen koennen.

## A-008 - Behebung zweier Flaky-Release-Gates (Coverage-Gate und E2E-Diagnose)

- **Befund:** Die Flakiness-Qualifikation (10 Wiederholungen, 0 Toleranz) scheiterte an zwei Stellen: (1) `coverage-gate` meldete in 1/10 Laeufen `critical pathUtils.js: line 96.85% branch 73.53% -> FAIL`; (2) die E2E-Suite scheiterte in 2/10 Laeufen mit 46/54 Szenarien, ohne dass der Name des fehlgeschlagenen Szenarios in den Logs auftauchte.
- **Ursache Coverage-Gate:** `canSymlink()` in tests/unit/src/pathUtils.test.js pruefte die Windows-Symlink-Faehigkeit mit EINEM einzigen `fs.symlinkSync`-Aufruf. Unter Last (Defender-Scan, ERROR_BUSY/ACCESS_DENIED) kann dieser Aufruf TRANSIENT fehlschlagen, obwohl Developer Mode aktiv ist. Ein Fehlschlag uebersprang alle sechs Symlink-Tests, wodurch die Walk-up-Zweige von `realIfExists` in src/pathUtils.js nicht mehr ausgefuehrt wurden und die Branch-Abdeckung unter den RR2-B003-Waiver-Floor (75 %) fiel. Zusaetzlich dokumentierte der Waiver-Scope in scripts/coverage-waivers.json die Branches `65:13.0` und `65:14.0` nicht, die LCOV in 10/10 Gate-Snapshots konstant als ungetestet misst - dadurch konnte der Scope-Honour-Mechanismus (der V8-Prozentrauschen abfangen soll) nie greifen.
- **Fix Coverage-Gate:** (a) `canSymlink()` versucht die Probe jetzt bis zu 5x mit Backoff; die Berechtigung selbst ist deterministisch, Retries koennen sie nicht vortaeuschen. (b) Der Waiver-Scope listet jetzt exakt die gemessene, stabile uncovered-Menge (65:12.0, 65:13.0, 65:14.0 ergaenzt). Verifikation: pathUtils steht danach stabil bei line 98.43 % / branch 80.65 % (4 Gate-Laeufe gruen, 12/12 Tests, 0 Skips).
- **Ursache E2E-Diagnose:** Der Flakiness-Runner filtert Suite-Ausgaben auf Zeilen mit 'fail'-Mustern; der vollstaendige JSON-Report zwischen E2E_BEGIN/E2E_END (mit Szenarionamen und Fehlermeldungen) wurde dadurch komplett verworfen - ein Flaky-Repetition war keinem Szenario zuzuordnen. 21 aufeinanderfolgende isolierte E2E-Laeufe (16 ohne Last, 5 direkt nach parallelen Unit-Suiten) waren gruen; der Fehler tritt nur im dichten Flakiness-Kontext auf.
- **Fix E2E-Diagnose:** scripts/e2e/run.js gibt bei jedem Szenario-Fehlschlag zusaetzlich eine kompakte Zeile `E2E_FAILED_SCENARIO: <name> :: <erste Fehlermeldung>` aus. Diese Zeile ueberlebt den Log-Filter des Flakiness-Runners, sodass jede kuenftige Flaky-Repetition direkt attribuierbar ist. Die E2E-Suite selbst wurde nicht geschwaecht (keine Checks entfernt, keine Timeouts erhoeht).
- **Verifikation:** Vollstaendige Flakiness-Qualifikation mit beiden Fixes gruen: serielle Unit-Suite + 10x alle 7 Release-Suiten (unit-parallel, smoke, e2e, contract, coverage-gate, lint, renderer-isolation) + 50x High-Risk-Batterie, 0 Fehlschlaege, FLAKY_EXIT=0.
- **Sicherheit:** Keine Schwaechung - die Coverage-Floors und Waiver-Regeln bleiben unveraendert streng; korrigiert wurden ein transienter Test-Skip und ein veralteter Scope-Eintrag sowie eine Diagnose-Luecke.


## A-009 - Fast-Forward-Merge von release/1.0.7-legacy nach main vor der Veroeffentlichung

- **Befund:** GitHub erlaubt workflow_dispatch ausschliesslich fuer Workflows, die auf dem Default-Branch (main) existieren. release-legacy-final.yml lag nur auf release/1.0.7-legacy; ein Dispatch war so unmoeglich.
- **Massnahme:** main (v1.0.6, dc75864) war direkter Vorfahre von release/1.0.7-legacy (2cc734f); der Merge erfolgte als reiner Fast-Forward ohne eigenen Merge-Commit und ohne Aenderung irgendeines Dateiinhalts. Danach Dispatch des Workflows mit --ref release/1.0.7-legacy und version=1.0.7 (Workflow-Laenge: Version ist hart auf 1.0.7 verriegelt).
- **Sicherheit:** Keine Schwaechung - die Veroeffentlichung bleibt unveraendert an die legacy-publication-Umgebung mit erforderlichem Reviewer gebunden; der Merge enthaelt ausschliesslich bereits qualifizierte, lokal gruen getestete Commits.


## A-010 - CI-Bereitstellung des kuratierten ffprobe.exe ueber die lock-verankerte npm-Abhaengigkeit

- **Befund:** Der erste CI-Lauf von release-legacy-final.yml scheiterte im Qualifikations-Job beim Schritt 'Setup offline runtime': Q-002 in scripts/setup.js bricht ohne kuratiertes bin/ffprobe.exe fail-closed ab. Lokal existiert die Datei, in einem frischen CI-Checkout nicht.
- **Ursache:** Das kuratierte ffprobe.exe ist kein Download des Setup-Skripts, sondern ein manuell verankertes Laufzeit-Asset (SHA-256-Pin in scripts/runtime-assets.json). Die frueheren Vollrelease-Workflows liefen nie 'npm run setup' in CI; der neue Qualifikations-Job folgt dagegen treu der Spezifikation 16.1 (setup + check).
- **Fix:** Beide CI-Jobs (qualify, candidate) stellen bin/ffprobe.exe jetzt VOR 'npm run setup' bereit, indem sie die win32-x64-Binaer der lock-verankerten Abhaengigkeit @ffprobe-installer/ffprobe kopieren (npm ci garantiert die Registry-Integritaet ueber package-lock.json). Zusaetzlich wird die Kopie fail-closed gegen den unabhaengigen SHA-256- UND Byte-Pin in scripts/runtime-assets.json geprueft.
- **Nachweis der Identitaet:** Das lokale kuratierte bin/ffprobe.exe ist bytetidentisch mit node_modules/@ffprobe-installer/win32-x64/ffprobe.exe (SHA-256 f28c4751e7367205267025aaf0fcfc921e34d9b7edaa46bd9c8abaf367fc9051, 80995328 Bytes, lokal verifiziert).
- **Sicherheit:** Keine Schwaechung - der Fail-closed-Mechanismus von Q-002 bleibt unveraendert; das Asset wird lediglich aus einer bereits integritaetsgesicherten Quelle bereitgestellt und doppelt (npm-Lock plus SHA-256-Pin) verankert. Der Legacy-Kandidat verschifft weiterhin KEIN ffprobe.exe (A-004/A-005 bleiben gueltig).


## A-011 CI: Developer Mode fuer Symlink-Faehigkeit im Qualifikations-Job

- **Befund:** CI-Lauf 31041943279 scheiterte in der Flakiness-Qualifikation (Repetition 6/10, coverage-gate): pathUtils.js Zeile 96.85% / Branch 73.53% - exakt dieselben Werte wie der lokale Flakiness-Fehler VOR dem A-008-Fix. Damit ist der Fehler in CI nicht transient, sondern deterministisch.
- **Ursache:** Der GitHub-Runner windows-latest hat KEINEN Developer Mode. Ohne das Symlink-Privileg liefert canSymlink() in tests/unit/src/pathUtils.test.js deterministisch false, alle sechs Symlink-Tests werden uebersprungen, und die Walk-up-Branches von realIfExists bleiben ungetestet - die Branch-Abdeckung faellt unter den RR2-B003-Waiver-Boden (75%). Der A-008-Retry fixt nur transiente Lastfehler, nicht ein fehlendes Privileg.
- **Fix:** Der Qualifikations-Job aktiviert VOR allen Testschritten den Developer Mode ueber den Registry-Wert AllowDevelopmentWithoutDevLicense=1 (HKLM AppModel Unlock; Runner laufen als Admin). Danach folgt ein fail-closed Assertion-Schritt, der den Lauf sofort abbricht, falls fs.symlinkSync weiterhin fehlschlaegt - ein stiller Skip der sechs Tests ist damit ausgeschlossen.
- **Sicherheit:** Keine Schwaechung des Coverage-Gates. Das Gate und die Waiver-Regeln bleiben unveraendert; die Tests laufen in CI jetzt tatsaechlich, statt deterministisch wegzuskippen. Es werden weder Defender noch SmartScreen noch Sicherheitsrichtlinien beruehrt - der Developer Mode aendert nur die Privilegienvergabe fuer symbolische Links auf der Wegwerf-CI-Maschine.

## A-012 CI: Junction-Fallback fuer die pathUtils-Link-Tests

- **Befund:** CI-Lauf 31047923259 scheiterte erneut in der Flakiness-Qualifikation (Repetition 1/10, coverage-gate) mit denselben Werten: pathUtils.js Zeile 96.85% / Branch 73.53%. Der A-011-Schritt (Developer Mode plus fail-closed Capability-Assertion) war gruen - das Privileg war vorhanden, aber unter CI-Last (Defender-Scans, parallele Prozesse) scheitert fs.symlinkSync TRANSIENT am Capability-Probe des Tests, und die sechs Link-Tests wurden in dieser Repetition weggelassen.
- **Ursache:** Die A-008-Retries (5 Versuche, kurze Backoffs) reichen unter CI-Last nicht immer aus; der Skip ist prozessweit und kostet die Branch-Abdeckung der realIfExists-Walk-up-Pfade.
- **Fix:** tests/unit/src/pathUtils.test.js nutzt jetzt makeDirLink(): ein dir-Symlink wird versucht, und NUR auf Windows faellt der Helper auf eine Directory-Junction zurueck (fs.symlinkSync mit Typ junction). Junctions brauchen kein Privileg und werden von realpathSync identisch aufgeloest - der getestete Escape-Schutz (realpath-Vergleich in pathUtils) wird damit auf demselben Codepfad geprueft. Auf nicht-Windows-Plattformen bleibt alles unveraendert capability-gesteuert.
- **Verifikation:** Lokal 12/12 Tests gruen (0 Skips). Zusaetzlicher Direktnachweis: eine Junction nach ausserhalb wird von isPathUnder/isParentUnderAny als Escape erkannt (false), eine Junction ins Root-Innere passiert (true), realpathSync loest die Junction exakt auf.
- **Sicherheit:** Keine Schwaechung - Coverage-Gate und Waiver-Regeln unveraendert; die Tests koennen unter Windows nicht mehr durch ein fehlendes Privileg wegskippen. Die CI-Assertion aus A-011 bleibt als Zusatzschutz bestehen.

## A-013 Seed-Release von Draft auf Pre-Release veroeffentlicht

- **Befund:** CI-Lauf 31051924586 scheiterte im Kandidaten-Job beim Schritt 'Download the pinned legacy seed release asset': gh release download meldete 'release not found'.
- **Ursache:** Die Seed-Release legacy-shell-seed-2026-08-04 war ein DRAFT. Das GITHUB_TOKEN des Workflows (contents: read) kann Draft-Releases nicht lesen.
- **Fix:** Die Seed-Release wurde von Draft auf veroeffentlicht gestellt und als PRE-RELEASE markiert, damit sie nicht als Produktrelease oder 'Latest' missverstanden wird. Der Release-Text stellt klar: internes Build-Input, kein Produktrelease, nicht installieren.
- **Sicherheit:** Keine Schwaechung - der Seed bleibt ueber die SHA-256-Pins in scripts/legacy-shell.lock.json verankert und wird von materialize-legacy-seed.ps1 fail-closed geprueft; die Oeffentlichkeit des Downloads aendert nichts an der Integritaetskette.

## A-014 CI: Fail-closed Link-Probe mit Diagnose statt stillem Skip

- **Befund:** CI-Lauf 31056203453 scheiterte erneut in Repetition 6/10 des coverage-gates mit denselben Werten (pathUtils.js 96.85% / Branch 73.53%), obwohl A-012 den Junction-Fallback bereits enthaelt. Ein anderer Lauf (31051924586) war mit identischem Code 10/10 gruen - der Probe-Schlag fehlt also nur INTERMITTIEREND, und der stille Skip verschleiert die wahre Ursache.
- **Fix:** Die Probe in tests/unit/src/pathUtils.test.js ist jetzt fail-closed mit Diagnose: (1) 10 Versuche mit wachsendem Backoff bis 2s statt 5 kurze, (2) kein sofortiges unlink mehr nach der Probe-Erzeugung (unter AV-Scans kann das unlink transient scheitern, obwohl die Erzeugung erfolgreich war - das faelschte bisher 'keine Faehigkeit'), (3) auf Windows WIRD NICHT MEHR GESKIPPT: scheitern alle Versuche, wirft die Suite einen harten Fehler mit den exakten OS-Fehlercodes aller Versuche. Auf nicht-Windows bleibt der graceful Skip erhalten.
- **Sicherheit:** Keine Schwaechung - im Gegenteil: ein Kapazitaetsverlust wird jetzt als lauter Testfehler sichtbar statt als stiller Skip mit Coverage-Fehlalarm. Gate und Waiver-Regeln unveraendert.

## A-015 CI: Coverage-Gate fail-closed Zaehler + einmalige Neu-Messung; E2E-Zoom mit Settle+Retry

- **Befund:** CI-Lauf 31059292879, Repetition 5/10: (1) coverage-gate FAIL mit exakt den Skip-Muster-Werten (pathUtils.js 96.85% / Branch 73.53%), OBWOHL die A-014-Probe nirgends geworfen hat (keinerlei Diagnose im 5.2-MB-Log) und die Unit-Suite mit Exit 0 durchlief; (2) E2E-Flake 'editor-tools :: Ctrl+- must zoom out (got 13.15, was 1.25)' - der Wert 13.15 ist exakt 1.25^11 * 0.8^2 bzw. ein Fit-Zoom-Wert, also eine async laufende Viewport-Aenderung, die ZWISCHEN den beiden Messungen landete.
- **Ursache (1):** Nodes V8-Coverage ist unter Last verlustbehaftet. Ein transienter Verlust der Branch-Daten einer Datei reproduziert EXAKT die uncovered-Menge weggeskippter Tests und hebelt damit das Scope-Honour-Sicherheitsnetz aus. Das Gate hat bisher eine unvollstaendige Messung still als reales Ergebnis gewertet.
- **Fix (1):** scripts/check-unit-coverage.js ist jetzt fail-closed zweistufig: (a) die Suite-Zaehler (tests/pass/fail/skipped/cancelled) werden aus dem Spec-Report geparst; skipped > 0 oder cancelled > 0 bricht das Gate hart ab - eine unvollstaendige Suite kann keine vertrauenswuerdige Critical-File-Tabelle liefern; (b) scheitert die Auswertung bei sauber gelaufener Suite (Exit 0, 0 Skips), wird die instrumentierte Suite EINMAL neu gemessen und auf der zweiten Messung geurteilt; die erste Fehlschlagliste wird protokolliert und im Evidence-JSON (suiteCounters, retried, firstAttemptFailures) festgehalten. Strukturelle Fehler (Suite-Crash, fehlende Summary, leeres LCOV) werden NIEMALS retried.
- **Ursache (2):** Eine stoerende async-Viewport-Aenderung (Refit/Zoom) konnte zwischen dem Tastendruck und dem Zoom-Read landen; der 20-ms-Sleep war unter CI-Last keine ausreichende Settle-Zeit.
- **Fix (2):** scripts/e2e/scenarios/editor-tools.js wartet jetzt, bis der Zoom sich nicht mehr aendert (3 stabile Messungen, max. 2 s Deadline), bevor er gewertet wird. Wird das Fenster dennoch korrupt gelesen, wird es aus Ctrl+1/Ctrl+= neu aufgebaut und der Zoom-out-Check bis zu 3x wiederholt. Ein echt kaputter Zoom-Out scheitert in allen 3 Versuchen - die Assertion bleibt in voller Staerke erhalten.
- **Sicherheit:** Keine Schwaechung - kein Floor wurde gesenkt, kein Waiver erweitert, keine Assertion abgeschwaecht. Die Neu-Messung kompensiert ausschliesslich die bereits im Gate dokumentierte Messinstabilitaet (KGO7-021 Noise-Note); eine echte Regression scheitert an beiden Messungen. Lokal verifiziert: Gate gruen (2719/2719, 0 skipped, pathUtils 98.43/80.65 OK), E2E-Szenario 4/4 gruen.

## A-016 CI: Watchdog gegen externe Lauf-Abbrueche des Release-Workflows

- **Befund:** Die Laeufe 31116994911, 31118935892 und 31123510412 von release-legacy-final wurden alle von aussen abgebrochen (Annotation exakt 'The operation was canceled', keine weitere Diagnose), ebenso der parallele CI-Lauf 31123602374. Bei 31123510412 war die komplette Qualifikation bereits ERFOLGREICH (inklusive 10x-Flakiness-Batterie und 50x-High-Risk); der Abbruch traf den Kandidaten-Job, waehrend er in der Runner-Warteschlange stand (0 ausgefuehrte Schritte). Saemtliche Gates bis zum jeweiligen Abbruch waren gruen.
- **Ursache:** Kein Test-Fehler, kein Timeout (Job-Limit ist 300 Minuten), kein Push- oder Dispatch-Ereignis im Zeitraum, kein manueller Abbruch (vom Nutzer ausdruecklich ausgeschlossen). Die Signatur entspricht Host-Evictions gehosteter Windows-Runner durch die Plattform - das liegt ausserhalb der Kontrolle des Repositories.
- **Fix:** Neues Workflow .github/workflows/release-legacy-watchdog.yml (workflow_run-Listener auf release-legacy-final, muss dafuer auf main liegen): Ein Retry erfolgt NUR, wenn KEIN Job ein echtes Failure-Verdikt traegt. Abgedeckt sind damit beide Varianten des verdiktlosen Infrastruktur-Abbruchs: Gesamt-Conclusion 'cancelled' sowie Gesamt-Conclusion 'failure' bei ausschliesslich cancelierten bzw. uebersprungenen Jobs (zweiter Fall: Qualifikation gruen, Kandidaten-Job in der Warteschlange evacuiert). In beiden Faellen werden die fehlgeschlagenen bzw. abgebrochenen Jobs DESSELBEN Laufs neu gestartet, mit Versuchslimit 3 insgesamt; ab Versuch 4 scheitert der Watchdog laut und verlangt menschliche Aufmerksamkeit. Traegt auch nur EIN Job die Conclusion 'failure', wird NIEMALS automatisch retried - ein rotes Gate bleibt Beweismittel und wird nicht durch einen zweiten Versuch uebermalt.
- **Sicherheit:** Keine Schwaechung - kein Gate, kein Floor, keine Assertion oder Waiver wird veraendert; der Retry fuehrt dieselbe Qualifikation auf demselben Head-SHA erneut aus. Eine automatische Veroeffentlichung entsteht nicht: Der Publish-Job bleibt an die geschuetzte Umgebung legacy-publication (Required Reviewer) gebunden und versendet ausschliesslich das vollstaendig qualifizierte Artefakt. Der Watchdog wird zusammen mit release-legacy-final nach Veroeffentlichung von v1.0.7 zurueckgezogen (Spezifikation Abschnitt 22).

## A-017 Seed-ZIP ohne resources/app.asar - Exporter-Fehler behoben, korrigierte Seed-Release

- **Befund:** Lauf 31127764095 von release-legacy-final scheiterte mit einem ECHTEN Verdikt (kein Infrastruktur-Abbruch; der Watchdog hat daher korrekterweise NICHT retried): Die Qualifikation war vollstaendig gruen, der Kandidaten-Job brach im Schritt 'Materialize the legacy seed' ab - materialize-legacy-seed.ps1 warf fail-closed 'Could not locate MiniMaxAssetTool.exe plus resources\app.asar'.
- **Ursache:** Die veroeffentlichte Seed-ZIP (legacy-shell-seed-2026-08-04) enthaelt KEINE resources/app.asar. Fehlerquelle ist scripts/capture-legacy-shell-lock.js: Die Exportmenge war ausschliesslich frozenFiles + peFiles, und app.asar liegt unter dem mutierbaren Praefix resources/app.asar - der Export liess sie weg. Eine Seed-Shell ohne app.asar ist aber kein lauffaehiges Layout; die Komposition ersetzt die asar zwar spaeter durch die Donor-asar, das Materialisieren verlangt die Datei jedoch zu Recht.
- **Fix:** (1) Der Exporter nimmt resources/app.asar jetzt IMMER in die Exportmenge auf (Kommentar im Code dokumentiert die Begruendung). (2) Neuer Regressionstest in tests/unit/scripts/legacyReleaseTools.test.js: der Seed-Export muss exe + app.asar + Lockdatei enthalten (6/6 gruen). (3) Korrigierte Seed-Release legacy-shell-seed-2026-08-06 veroeffentlicht (Pre-Release, internes Build-Input, kein Produktrelease): identische gefrorene/PE-Dateien laut scripts/legacy-shell.lock.json (Re-Capture gegen die lokale Referenz bestaetigt die Lock-Inhalte; die Quelle enthaelt seit der Nutzung zusaetzliche Laufzeit-Zustandsdateien batches.json/state.json, die korrekterweise NICHT Teil des Seeds sind) plus die originale 1.0.0-app.asar (SHA-256 6c0b380a5bdde7ff4d61f189423a5a86707de4cac96b61c0c3a27652a7985a5a). ZIP-SHA-256: ddb455fb47b402b5ad92a40717ae6b4003e83bac9c137a34c85e882948d4d19e. (4) SEED_RELEASE_TAG im Workflow auf die neue Markierung gesetzt. Die alte Seed-Release bleibt entsprechend der Immutable-Tag-Regel unangetastet.
- **Verifikation:** End-zu-Ende lokal simuliert: die ZIP passiert materialize-legacy-seed.ps1 (seed_path wird exportiert) und die materialisierte Shell besteht verifyFileMap gegen frozenFiles UND peFiles der Lockdatei. Die Seed-asar selbst wird beim Versand NIEMALS ausgeliefert - die Komposition ersetzt sie durch die frisch gebaute Donor-asar; sie dient nur als vollstaendiges Shell-Layout.
- **Sicherheit:** Keine Schwaechung - das fail-closed-Materialisieren hat den Fehler beweisbar abgefangen; kein Gate, keine Lock-Datei und kein Floor wurden veraendert. Die Lock-Inhalte sind byte-identisch bestaetigt (Abweichung ausschliesslich createdAt sowie zwei nicht exportierte Laufzeit-Zustaende).

## A-018 Installer: Integritaetspruefung ohne Get-FileHash (Modul-Aufloesung im verschachtelten PowerShell)

- **Befund:** Lauf 31128913417 von release-legacy-final scheiterte mit einem ECHTEN Verdikt (kein Infrastruktur-Abbruch; der Watchdog hat korrekterweise NICHT retried): Die Qualifikation war gruen, die Seed-Korrektur aus A-017 hat gewirkt (Materialisieren, Komposition und Donor-asar alle gruen). Der Kandidaten-Job brach im Schritt 'Package the composed candidate' ab: scripts/test-release-installer.js meldete im Integrity-Loop des Installers 'Get-FileHash : The term Get-FileHash is not recognized' (CommandNotFoundException); die Folgefehler in Upgrade- und Source-equals-install-Test waren eine KASKADE des ersten Fehlers (die Installation wurde nie vollzogen, daher fehlte die getauschte Datei), keine eigenstaendigen Fehler.
- **Ursache:** Die eingebetteten PowerShell-Integritaetsschleifen in 'Install MiniMax Asset Tool.cmd' nutzten das Cmdlet Get-FileHash aus dem Modul Microsoft.PowerShell.Utility. In der CI-Umgebung der pwsh-7-Schritte konnte das verschachtelte powershell.exe 5.1 dieses Modul nicht aufloesen. Lokal reproduzierbar gemacht durch verschmutztes PSModulePath.
- **Fix:** Alle drei Get-FileHash-Stellen (Staging-FILES.sha256-Loop, Verifier-SHA-256-Pin im Bootstrap, Part-Checksum-Loop) wurden durch flache .NET-Sequenzen ersetzt: [System.Security.Cryptography.SHA256]::Create() mit ComputeHash ueber einen Dateistream - ohne jegliche Modulabhaengigkeit. Der .NET-Hashwert ist nachweislich identisch mit der Get-FileHash-Ausgabe. Keine Assertion wurde geaendert: Manipulations-Erkennung, Vollstaendigkeitspruefung, Manifest-Mindestgroesse, Fail-closed-Verhalten und die RR2-C001-Vertrauensanker bleiben byte-fuer-byte wirksam.
- **Verifikation:** Lokal 7/7 Installer-Tests gruen, sowohl normal als auch unter CI-simuliertem verschmutztem PSModulePath; separater Hash-Gleichheits-Nachweis (dotnet == Cmdlet) erbracht. verify-release.js wurde geprueft und bewusst NICHT geaendert: sein verschachtelter Get-AuthenticodeSignature-Aufruf ist im Legacy-Modus (verify:release:legacy ohne --require-authenticode/--require-signature) rein berichtend und niemals gate-relevant.
- **Sicherheit:** Keine Schwaechung - kein Gate, kein Floor, keine Assertion oder Waiver wurde veraendert; die Integritaetskette (Minisign-Signatur, SHA-256-Inventar, FILES.sha256-Vollstaendigkeit) bleibt in voller Staerke erhalten und wird nun von einer Modul-unabhaengigen Implementierung geprueft.

## A-019 E2E: resilience-disk Fehleranzeige-Check mit Frist-Polling statt Einzel-Schnappschuss

- **Befund:** Lauf 31131194919 von release-legacy-final scheiterte mit einem ECHTEN Verdikt des Flakiness-Gates: Qualifikations-Repetition 4/10 meldete 'resilience-disk: non-existent output_dir did not produce a visible error element' (46/54 Checks), alle uebrigen 9 Repetitionen sowie die komplette 50x-High-Risk-Batterie waren gruen. Das Gate hat den Flake korrekt als Freigabe-Hindernis gewertet.
- **Ursache:** Der QA-031-Check in scripts/e2e/scenarios/resilience-disk.js beurteilte die Fehleranzeige in einem EINZIGEN Schnappschuss nach DELAY+500 ms. Der asynchrone Fehlerpfad (Klick -> Validierung -> Toast) kann unter CI-Last laenger dauern; der Toast selbst bleibt 3-4 s sichtbar. Ein spaetes, aber korrektes Fehler-UI wurde dadurch faelschlich als fehlend gewertet.
- **Fix:** Der Check pollt jetzt IN-PAGE mit Frist (max. 6 s, 100-ms-Takt, analog zum Settle+Retry-Muster aus A-015 und i18n-input): Erscheint Toast oder Fehler-Panel innerhalb der Frist, gilt die Assertion als erfuellt; erscheint die Fehleranzeige UEBERHAUPT NICHT, scheitert der Check weiterhin hart. Die Assertion selbst (QA-031: sichtbares Fehler-UI statt bloss 'kein Absturz') bleibt in voller Staerke bestehen.
- **Verifikation:** Lokal 3x komplette E2E-Suite gruen (je 47/47, 0 Szenario-/Visual-Fehler, exit 0), resilience-disk in allen Laeufen bestanden.
- **Sicherheit:** Keine Schwaechung - kein Floor (surface-threshold 96) und keine Gate-Logik wurden veraendert; das Flakiness-Gate verlangt weiterhin 10/10 gruen Repetitionen.

## A-020 CI: Fail-closed Electron-Installationspruefung mit deterministischer Reparatur nach npm ci

- **Befund:** Lauf 31134617191 von release-legacy-final scheiterte in der Qualifikation bei 'Unit tests (with line coverage)': exakt 1 von 2720 Tests ('fb:rename rejects newName with path separators') meldete 'Electron failed to install correctly. Please delete node_modules/electron and run npx install-electron'. Die identische Suite war unmittelbar zuvor lokal dreimal in Folge 2720/2720 gruen (Commits 9d5f3c7 und 22ab9a0, Pre-commit-Gate) - der Fehler ist eine beschaedigte Runner-Umgebung, kein Code-Fehler.
- **Ursache:** npm ci laesst node_modules/electron UNVOLLSTAENDIG zurueck, wenn der Post-Install-Download der Electron-Binaerdatei unterbrochen wird (bekanntes transientes Verhalten gehosteter Runner). Ohne Pruefung vergiftet das die Unit-Gates mit einem Umweltfehler, der wie ein echtes Test-Verdikt aussieht.
- **Fix:** Neuer Workflow-Schritt 'Assert Electron install integrity (A-020)' unmittelbar nach jedem npm ci (Qualifikation und Kandidat): eine fail-closed Sonde (node -e "require('electron')") prueft die Installation; schlaegt sie fehl, folgt GENAU EINE deterministische Reparatur (node node_modules/electron/install.js - der offizielle Installer des Paketinhalts, derselbe Codepfad wie der npm-Postinstall). Bleibt die Installation danach weiter beschaedigt, bricht der Job laut ab - die Gates laufen niemals auf einer kaputten Abhaengigkeit. Der Publish-Job braucht kein npm ci und ist unveraendert.
- **Verifikation:** YAML-Syntax validiert (js-yaml-Parse), Sondenlogik lokal gruen (require('electron') aufgeloest), Schritt in beiden betroffenen Jobs vorhanden.
- **Sicherheit:** Keine Schwaechung - kein Gate, kein Floor und keine Assertion wurden veraendert; die Sonde macht einen Umweltfehler FRUEH und UNTRENKBAR von echten Test-Verdikten sichtbar, statt ihn erst in den Unit-Gates als Einzeltest-Fehler sichtbar zu machen.

## A-021 Installer-Testharness: Shortcut-Sonde mit Fail-closed-Diagnose statt stillem Fehlurteil

- **Befund:** Lauf 31135143916 von release-legacy-final: Die Qualifikation war VOLLSTAENDIG GRUEN (A-019 und A-020 haben gewirkt). Der Kandidaten-Job scheiterte bei 'Package the composed candidate' mit exakt EINEM Fehler: 'shortcut does not point to the packaged executable' - waehrend alle sechs anderen Installer-Tests liefen, einschliesslich des kompletten Signatur-Gates. Die A-018-Korrektur hat also gewirkt (alle Integritaets-Loops gruen); der neue Fehler liegt im TESTHARNESS, nicht im Installer: Installation, Shortcut-Erzeugung und .lnk-Existenz waren alle erfolgreich (sonst haette es andere Fehlermeldungen gegeben).
- **Ursache:** Die Sonde in scripts/test-release-installer.js liest das Shortcut-Ziel ueber ein verschachteltes powershell.exe (WScript.Shell COM) ohne ErrorActionPreference und ohne jede Diagnose. Jeder Sondenfehler (stille COM-Stoerung, leere Ausgabe, abweichender Pfad) landete in EINER undifferenzierten Meldung ohne Soll-/Ist-Werte. Dieselbe Fehlerklasse wie A-018: ein verschachteltes PowerShell ohne Umgebungsrobustheit, hier verschleiert durch fehlende Diagnose. (Die wahre Ursache des Laufes 31135143916 war eine Pfad-Schreibweisen-Divergenz - sie wurde erst durch die A-021-Diagnose im Folgetauf beweisbar, siehe A-022.)
- **Fix:** Die Sonde laeuft jetzt mit $ErrorActionPreference='Stop' (aus einem stillen $null wird ein harter Abbruch mit stderr) und das Harness unterscheidet DREI fail-closed Faelle mit exakter Diagnose: (1) Sonden-Prozess scheitert -> Exit-Code und stderr werden gemeldet; (2) leeres TargetPath -> wird als leer gemeldet (nicht als falsches Ziel); (3) wirklich abweichendes Ziel -> Soll- und Ist-Wert werden gemeldet. Ein kaputter Shortcut scheitert weiterhin hart - es gibt keinen Weg, ein falsches Ziel durchzulassen.
- **Verifikation:** Lokal 7/7 Installer-Tests gruen (no-admin/Shortcuts, Upgrade, Source-equals-install, Manipulations-Ablehnung, Multipart, Unsigned-Ablehnung, Verifier-Ablehnung).
- **Sicherheit:** Keine Schwaechung - die Shortcut-Assertion bleibt in voller Staerke bestehen (falsches Ziel scheitert weiterhin); geaendert wurde nur die Diagnosefaehigkeit der Sonde, damit ein Umweltfehler nicht mehr als Produktfehler getarnt wird.

## A-022 Installer-Testharness: Shortcut-Vergleich auf kanonischen Pfaden (8.3-Kurzname gegen Langname)

- **Befund:** Lauf 31190666688 von release-legacy-final: Qualifikation erneut VOLLSTAENDIG GRUEN; der Kandidaten-Job scheiterte wieder bei exakt derselben Assertion - diesmal MIT der A-021-Diagnose: 'target: C:\Users\runneradmin\AppData\Local\Temp\...\MiniMaxAssetTool.exe, expected: C:\Users\RUNNER~1\AppData\Local\Temp\...\MiniMaxAssetTool.exe'. Beide Angaben bezeichnen DIESELBE Datei: Der gehostete Runner liefert os.tmpdir() als 8.3-Kurzname (RUNNER~1), waehrend WScript.Shell den Shortcut-TargetPath beim Speichern auf den kanonischen Langnamen normalisiert. Der Installer und der Shortcut sind korrekt - der String-Vergleich des Harness war zu streng fuer Windows-Pfad-Schreibweisen.
- **Ursache:** path.resolve() aendert keine Schreibweise; auf Systemen mit 8.3-Kurznamen (oder abweichender Gross-/Kleinschreibung) koennen zwei gueltige Schreibweisen derselben Datei den String-Vergleich verlieren. Dies ist eine Testharness-Schwaechung der Vergleichsmethode, KEIN Produktfehler: Die A-021-Diagnose zeigt, dass der Shortcut exakt auf die installierte Anwendung zeigt.
- **Fix:** Der Vergleich normalisiert beide Seiten mit fs.realpathSync.native (Windows-kanonische Form: Kurznamen aufgeloest, Schreibweise vereinheitlicht). realpathSync.native WIRFT bei nicht existierenden Pfaden - ein fingiertes oder kaputtes Shortcut-Ziel scheitert daher weiterhin fail-closed mit genauer Meldung, statt still durchzugehen.
- **Verifikation:** (1) Lokal 7/7 Installer-Tests gruen. (2) Direkter Beweis der Divergenz: ein per WScript.Shell mit abweichender Schreibweise gespeicherter Shortcut meldet beim Ruecklesen die kanonische Schreibweise (Sonde scratch_verify/_a022probe.ps1); fs.realpathSync.native bewertet beide Schreibweisen als gleich.
- **Sicherheit:** Keine Schwaechung - die Assertion 'Shortcut zeigt auf die installierte Anwendung' bleibt in voller Staerke bestehen; veraendert wurde ausschliesslich die Vergleichsmethode von rohen Pfad-Strings auf kanonische Dateisystem-Identitaet.

## A-023 Minisign-Secrets: Neuer Schluesselsatz nach ungueltigem MINISIGN_KEY_B64 + Fail-closed-Diagnose in prepare-minisign.ps1

- **Befund:** Lauf 31195906036 von release-legacy-final: Qualifikation zum DRITTEN Mal in Folge VOLLSTAENDIG GRUEN (A-022 hat gewirkt - der Installer-Test lief fehlerfrei durch). Der Kandidaten-Job scheiterte erstmals im Schritt 'Prepare pinned Minisign toolchain': [Convert]::FromBase64String meldete 'The input is not a valid Base-64 string'. WICHTIG: Dieser Schritt wurde in den Laeufen 31135143916 und 31190666688 NIE erreicht, weil der Kandidaten-Job jeweils vorher am Installer-Test abbrach - dies war die ERSTE echte Ausfuehrung des Schrittes. Das Geheimnis MINISIGN_KEY_B64 (zuletzt gesetzt am 2026-08-04) enthaelt demnach kein gueltiges Base64; ein transitiver Fehler ist ausgeschlossen, weil derselbe Wert in jeder Ausfuehrung identisch injiziert wird.
- **Ursache:** Der urspruengliche private Schluessel wurde planmaessig nach dem Setzen der Geheimnisse lokal geloescht (Spezifikation Schritt 8) und ist nicht wiederherstellbar. Da der defekte Wert nicht auslesbar ist (GitHub-Geheimnisse sind write-only) und noch KEIN einziges Artefakt mit diesem Schluessel signiert wurde (keine .minisig-Dateien existieren, keine veroeffentlichte Version), ist die saubere Loesung ein neuer Schluesselsatz statt einer forensischen Rekonstruktion.
- **Fix:** (1) Neuer Minisign-0.11-Schluesselsatz aus dem hashgepinnten Archiv (SHA-256 B9C31C2C...58031 lokal verifiziert) mit leerem Passwort erzeugt - sign-release.js ruft minisign -S ohne Passwortuebergabe auf, das verlangt zwingend ein leeres Passwort. (2) Rundlauf-Test lokal gruen: Signatur und Verifikation eines Testdokuments mit exakt den CI-Aufrufmustern. (3) Die exakte Upload-Zeichenkette wurde lokal durch [Convert]::FromBase64String dekodiert und Byteweise mit der Quelldatei verglichen (ROUNDTRIP OK), BEVOR sie als Geheimnis gesetzt wurde. (4) Beide Repository-Geheimnisse neu gesetzt (MINISIGN_KEY_B64, MINISIGN_PUB_KEY); der lokale private Schluessel wurde anschliessend geloescht. Neuer oeffentlicher Schluessel: RWRaquGIdyhfyEZfBqwiGnq8F+SFGJkPam1nnmtvIi2wVN7ALS7i7mTJ (ersetzt RWQt5uv... in scratch_verify/minisign-pinned.pub; kein versionierter Quellcode enthaelt den alten Schluessel - der Installer-Stempelpunkt RR2-C001 wird erst bei finalize:release aus dem Geheimnis befuellt). (5) prepare-minisign.ps1 prueft jetzt VOR der Dekodierung fail-closed mit Diagnose: Leer-Check, Laenge modulo 4, Base64-Alphabet - ein defektes Geheimnis erzeugt kuenftig eine verstaendliche Meldung statt einer rohen .NET-Exception.
- **Verifikation:** Rundlauf-Signatur lokal gruen (sign exit 0, 'Signature and comment signature verified'); Base64-Rundlauf bytetreu; gh secret list zeigt beide Geheimnisse mit neuem Zeitstempel 2026-08-07; private Schluesseldatei geloescht (nur minisign.pub verbleibt im Temp-Verzeichnis).
- **Sicherheit:** Keine Schwaechung - Minisign bleibt reiner Integritaetsschutz, das RR2-C001-Vertrauensanker-Verfahren (gepinnter Verifier + eingebetteter oeffentlicher Schluessel im Installer) bleibt unveraendert; der alte Schluessel war nie produktiv im Einsatz, daher entsteht kein Vertrauensbruch. Die Geheimnis-Werte selbst wurden zu keinem Zeitpunkt protokolliert oder versioniert.

## A-024 Minisign-Aufloesung: sign-release.js und verify-release.js hoeren auf MINISIGN_TOOL_PATH (Node-spawnSync ohne PATHEXT)

- **Befund:** Lauf 31202339704 von release-legacy-final: Qualifikation zum VIERTEN Mal in Folge VOLLSTAENDIG GRUEN; der A-023-Schluesselsatz hat gewirkt ('Prepare pinned Minisign toolchain' lief fehlerfrei durch, die fail-closed-Base64-Pruefung ebenfalls). Der Kandidaten-Job scheiterte im FOLGESCHRITT 'Sign the release inventory': 'ERROR: minisign binary not found', obwohl der Schritt die Umgebungsvariable MINISIGN_TOOL_PATH=D:\a\_temp\minisign-0.11\minisign-win64\minisign.exe trug - die Datei existierte nachweislich (der vorherige Schritt hatte sie erzeugt und verifiziert).
- **Ursache:** findMinisign() in scripts/sign-release.js kannte MINISIGN_TOOL_PATH ueberhaupt nicht (obwohl prepare-minisign.ps1 die Variable exakt fuer diesen Zweck exportiert) und verliess sich auf spawnSync('minisign', ['-V']). Zwei Fehler ueberlagerten sich: (1) Node-spawnSync zieht unter Windows KEIN PATHEXT heran - der blosse Name 'minisign' wird nicht zu 'minisign.exe' aufgeloest; (2) die Sonde 'minisign -V' ist das Verifikationsverb und beendet sich OHNE Argumente immer mit einem Usage-Fehler (exit 2) - selbst ein korrekt aufgeloester Binary haette die Sonde verloren. Dieselbe kaputte Sonde steckte auch in scripts/verify-release.js (AUD-015-Gate '--require-manifest-signature'), das den Kandidaten im naechsten Schritt ebenfalls falsch zum Scheitern gebracht haette.
- **Fix:** Beide Skripte loesen den Verifier jetzt identisch auf: (1) MINISIGN_TOOL_PATH ist der KANONISCHE Zeiger - gesetzt, aber unbrauchbar, scheitert das Skript fail-closed mit Diagnose (ein stiller Rueckgriff auf einen ANDEREN minisign wuerde die Pin-Bindung brechen); (2) PATH-Aufloesung prueft beide Schreibweisen ('minisign' und 'minisign.exe') mit einer echten Lebens-Sonde: ein erfolgreicher Spawn ohne ENOENT/EACCES beweist den Binary, unabhaengig vom Usage-Exit-Code; (3) erst danach folgen die bekannten Installationsorte. In verify-release.js fuehrt ein unauffindbarer Verifier zu einem harten Fehler ('manifest signature cannot be verified') - das Signatur-Gate wird NIEMALS still uebersprungen.
- **Verifikation:** Lokal: (1) mit MINISIGN_TOOL_PATH auf den hashgepinnten 0.11-Binary loest das Skript den Binary auf und scheitert erst am lokal fehlenden 1.0.7-Manifest (Beweis der Aufloesung); (2) mit einem Phantom-Pfad scheitert es fail-closed mit exakter Meldung; (3) ohne Variable greift der lokale Fallback wie bisher.
- **Sicherheit:** Keine Schwaechung - das AUD-015-Signatur-Gate bleibt in voller Staerke erforderlich; geaendert wurde nur die Binaer-Aufloesung von 'prinzipiell unmoeglich' auf 'deterministisch und fail-closed'. Die Verifikation selbst (-V -p -m -x) ist unveraendert.

## A-025 Provenance-Gate: seed-download/ und legacy-seed/ in .gitignore (Pipeline-Zwischenverzeichnisse)

- **Befund:** Lauf 31207377990 von release-legacy-final: Qualifikation zum FUENFTEN Mal in Folge VOLLSTAENDIG GRUEN; A-024 hat gewirkt ('Sign the release inventory' lief fehlerfrei, die Signatur wurde verifiziert: 'Manifest: verified'). Der Kandidaten-Job scheiterte im Folgeschritt 'Verify the release (legacy mode)' mit exakt EINEM Fehler: 'Provenance says the release was built from a dirty or unknown Git tree'. Alle uebrigen Provenance-Pruefungen (Commit-Identitaet, Electron-Version, asar-Hash, Frische) waren gruen.
- **Ursache:** zip-portable.js zeichnet beim Paketieren commitDirty aus git status --porcelain auf. Der Kandidaten-Job erzeugt VOR dem Paketieren die Arbeitsverzeichnisse seed-download/ (Schritt 'Download the pinned legacy seed release asset') und legacy-seed/ (Schritt 'Materialize the legacy seed') im Worktree - beide waren NICHT in .gitignore und zaehlten daher als ungetrackte Dateien. Die Vergleichsverzeichnisse dist-work/ und dist-out/ waren bereits ignoriert; diese beiden fehlten. Dieselbe Fehlerklasse wie RQ-002 (Signiermaterial im Worktree): Das Provenance-Gate hat KORREKT fail-closed gewertet - der Fehler liegt in der Pipeline-Hygiene, nicht im Gate.
- **Fix:** .gitignore ignoriert jetzt seed-download/ und legacy-seed/ mit erklaerendem Kommentar. Das Gate selbst ist UNVERAENDERT: commitDirty !== false scheitert weiterhin hart. Lokal verifiziert: mit erzeugten Probeverzeichnissen meldet git status --porcelain keine der beiden Verzeichnisse mehr.
- **Verifikation:** git status --porcelain mit angelegten Probeverzeichnissen: 0 Treffer fuer seed-download|legacy-seed; die restlichen Eintraetze sind bereits existierende lokale Scratch-Dateien (auf CI entsteht ein frischer, sauberer Checkout).
- **Sicherheit:** Keine Schwaechung - das Provenance-Gate bleibt in voller Staerke bestehen (schmutziger Baum scheitert weiterhin); geaendert wurde nur, dass reiner Pipeline-Zwischenspeicher nicht mehr als Quelltext-Aenderung gezaehlt wird.

## A-026 Publish-Job: getesteten Baum aus den versandten Archiven rekonstruieren (verify:release:legacy braucht win-unpacked)

- **Befund:** Lauf 31212811864 von release-legacy-final: Qualifikation zum SECHSTEN Mal in Folge VOLLSTAENDIG GRUEN und der Kandidaten-Job erstmals KOMPLETT GRUEN (alle 24 Schritte einschliesslich Signatur, Verifikation, Provenance, SBOM, Signing-Scope, drei Acceptance-Suiten und Staging). Der Publish-Job scheiterte nach erfolgter Environment-Freigabe im Schritt 'Verify the staged publication before shipping': 'Missing executable: dist-out\win-unpacked\MiniMaxAssetTool.exe' und 'Provenance recorded an asar hash but app.asar is missing'. Manifest und Minisign-Signatur waren zu diesem Zeitpunkt bereits verifiziert ('Manifest: verified').
- **Ursache:** Das Kandidaten-Artefakt enthaelt definitionsgemaess NUR das Publikationsinventar (dist-out/publication/*: partN.zip, SHA-256-Manifest, .minisig, Provenance, SBOM). verify:release:legacy prueft zusaetzlich den entpackten Baum (EXE-Existenz, app.asar-Hash gegen Provenance) unter dist-out/win-unpacked - dieser lag im Publish-Job nicht vor, weil der Kandidaten-Job ihn nicht in das Artefakt legt. Der Fehler ist eine Luecke im Publish-Ablauf, KEIN Gate-Fehler: Das Gate hat korrekt fail-closed gewertet.
- **Fix:** Der Publish-Job rekonstruiert dist-out/win-unpacked jetzt VOR der Verifikation aus EXAKT den Archiven, die veroeffentlicht werden: jede .partN.zip (laut QA-023 unabhaengige, einzeln gueltige ZIPs) wird unter RUNNER_TEMP entpackt, der Baum MiniMaxAssetTool-1.0.7-x64 nach dist-out/win-unpacked verschoben und fail-closed auf MiniMaxAssetTool.exe sowie resources/app.asar geprueft. Das verstaerkt die Garantie gegenueber dem Kandidaten-Job: Es wird bewiesen, dass die zu veroeffentlichenden ZIPs den getesteten Baum enthalten (Aequivalent der S21-Testextraktion), und der Provenance-asar-Hash wird gegen die TATSAECHLICH versandten Bytes geprueft.
- **Verifikation:** Die Extraktionslogik entspricht dem bereits in CI gruen gelaufenen Installationspfad (Install-CMD und Installer-Tests entpacken dieselben partN.zip-Formate); die Rekonstruktion erfolgt ausschliesslich unter RUNNER_TEMP bzw. im gitignorierten dist-out (kein Einfluss auf das Provenance-Kriterium commitDirty, da der Publish-Job am selben SHA 14e8faf ausgecheckt ist). Der naechste Lauf beweist die Kette endgueltig.
- **Sicherheit:** Keine Schwaechung - kein Gate wurde abgeschaltet oder abgeschwaecht; die Verifikation laeuft unveraendert mit --require-archive --require-manifest-signature --require-provenance --require-sbom und erhaelt jetzt alle benoetigten Eingaben aus dem exakt getesteten Artefakt.

## A-027 Seed-Release-Notes versehentlich ueberschrieben und rekonstruiert (kein Asset veraendert)

- **Befund:** Beim Anhaengen des Archiv-Vermerks an die Release-Notes von legacy-shell-seed-2026-08-06 schlug der erste gh-Aufruf fehl ('--template' ohne '--json'), wodurch die bestehende Body lokal nicht gesichert wurde und gh release edit die Notes durch den reinen Archiv-Vermerk ersetzte.
- **Ursache:** Operativer Fehler im Archivierungsschritt von S22; die urspruengliche Body war in keiner lokalen Datei und keinem Transkript wiederherstellbar.
- **Fix:** Die Notes wurden aus den belegbaren Fakten rekonstruiert (Tag, Asset MiniMaxAssetTool-1.0.0-legacy-seed-x64.zip, Herkunft C:\Tools\MinimaxAssetTool1.0.0, korrigierter Seed A-017/e431e77 mit ZIP ddb455fb...d4d19e, Lockfile-Bezug, Unveraenderlichkeits-Policy) und um den Archiv-Vermerk ergaenzt. Tag und Assets des Seed-Releases wurden zu keinem Zeitpunkt veraendert.
- **Sicherheit:** Kein Gate betroffen; es handelt sich ausschliesslich um Release-Beschreibungstext. Die Unveraenderlichkeit von Tag und Artefakten blieb gewahrt.

## A-028 Ausserplanmaessige 1.0.8-Hotfix-Release (Extraktions-Vorfall bei v1.0.7)

- **Befund:** Beim Endanwender startete das veroeffentlichte v1.0.7 nach dem Doppelklick auf MiniMaxAssetTool.exe nicht; debug.log zeigte wiederholt 'FATAL:gin\v8_initializer.cc:681 Error loading V8 startup snapshot file'. Ursache: Die Installation enthielt 100% von part1.zip, aber 0% von part2.zip (darin lagen snapshot_blob.bin, v8_context_snapshot.bin, resources.pak, Vulkan-Bestandteile und alle Modelle). Die veroeffentlichten Artefakte selbst waren intakt: Die Downloads des Anwenders sind byte- und hashidentisch mit den GitHub-Assets, und eine frische Extraktion BEIDER Parts ergibt eine vollstaendige, gegen FILES.sha256 verifizierte Installation (3406/3406), die startet.
- **Ursache:** QA-023 teilt Releases wegen der 2-GiB-Grenze in UNABHAENGIGE Part-ZIPs; alle Parts muessen in denselben Ordner entpackt werden. Das Entpacken von part2 scheiterte beim Anwender still (Windows Explorer bricht grosse ZIPs ohne Fehlermeldung ab) — und ausgerechnet die Chromium-Startdateien lagen in part2, wodurch die EXE ohne jeden Dialog stirbt. Prozessluecke: Die Release-Verifikation pruefte Archiv-Integritaet (Hashes, Minisign — alles korrekt), aber es gab keinen Endanwenderpfad-Test 'beide Parts frisch entpacken und starten'.
- **Fix (Release 1.0.8, explizite Anweisung des Maintainers):** (1) zip-portable.js pinnt alle boot-kritischen Dateien (EXE, V8-Snapshots, pak-Dateien, Rendering-DLLs, resources/app.asar, FILES.sha256, verify-install.*) in part1 und scheitert fail-closed, falls die Pinning-Invariante nicht haelt; eine part1-Extraktion allein kann dadurch booten. (2) Das Paket enthaelt jetzt verify-install.cmd/verify-install.ps1 (Node-frei), das jede Datei gegen FILES.sha256 prueft und fehlende/beschaedigte Dateien exakt benennt. (3) START HERE und Release-Notes dokumentieren das zwingende 'ALLE Parts in denselben Ordner'-Verfahren. (4) Der CI-Kandidaten-Job enthaelt zusaetzliche harte Gates: part1-Bootdatei-Check und ein Clean-Extract-Boot-Test (beide Parts frisch entpacken, EXE starten, Prozess lebend, kein FATAL).
- **Abweichung vom Migrationsplan:** Der Plan sah vor, dass 1.0.7 die letzte Legacy-Line-Release bleibt und die naechste Version 1.1.0 (SignPath) ist. Auf explizite Anweisung des Maintainers wird 1.0.8 als reine Packaging-Hotfix-Release dazwischengeschoben: identischer Anwendungsstand wie 1.0.7, direkt aus Quelltext gebaut (keine Legacy-Shell-Komposition), Integritaetsmodell unveraendert (Minisign + SHA-256, KEIN Authenticode). Die 1.1.0-SignPath-Linie bleibt davon unberuehrt.
- **Sicherheit:** Kein Gate wurde abgeschwaecht. v1.0.8 durchlaeuft die volle Qualifikation (Lint, Unit + Coverage-Gate, Contract, IPC-Coverage, Mutation, Flakiness) sowie Signatur/Verifikation/Acceptance unveraendert; die neuen Extraktions-Gates verschaerfen die Pipeline zusaetzlich. Tags bleiben unveraenderlich: v1.0.7 wird nicht angeruehrt, 1.0.8 erhaelt einen eigenen Tag.
