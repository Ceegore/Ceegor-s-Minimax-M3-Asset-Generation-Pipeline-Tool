...
- **Ursache:** Die Sonde in scripts/test-release-installer.js liest das Shortcut-Ziel ueber ein verschachteltes powershell.exe (WScript.Shell COM) ohne ErrorActionPreference und ohne jede Diagnose. Gibt die Sonde nichts aus (z. B. weil COM in der Runner-Sitzung still scheitert und der Eigenschaftszugriff auf $null leer bleibt), wird das stille Leer-Ergebnis als 'falsches Ziel' gewertet: path.resolve('') ist das Arbeitsverzeichnis, niemals das erwartete Ziel. Dieselbe Fehlerklasse wie A-018: ein verschachteltes PowerShell ohne Umgebungsrobustheit, hier verschleiert durch fehlende Diagnose.
